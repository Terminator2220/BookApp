...
function BookApp() {
  ...
  return (
    <div className='conteiner'>
      ...
      <input type="text" placeholder="Пошук..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className='inpSerch'/>
      <ul>
        {books.filter((book) =>
          ...
          .map((book) => (
            <li key={book.id}>
              ...
              <button onClick={() => deleteBook(book.id)} className='delButton'><img src="./svg/trash.svg"/></button>
              {book.read && <span>✔</span>}
            </li>
          ))}
      </ul>
